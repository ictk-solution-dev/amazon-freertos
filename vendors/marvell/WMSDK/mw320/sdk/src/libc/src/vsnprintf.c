...
#define MAX_DEFAULT_FLOAT_PREC	6
...
static size_t
format_float(char *q, size_t n, float val, unsigned int flags,
	   int width, int prec)
{
	int precision = (prec == -1 || prec > MAX_DEFAULT_FLOAT_PREC ?
			MAX_DEFAULT_FLOAT_PREC : prec);
	int scale = 1;
	size_t o = 0;
	...
	int int_part_of_float = (int)val;
	int frac_part_of_float = (int)((val - (int)val) * scale);

	/* Keep fractional part positive. We don't need to print sign for it */
	if (frac_part_of_float < 0) {
		frac_part_of_float = -frac_part_of_float;
		/* This is to handle a special case of negative float values
		 * greater than -1.
		 * We emit negative sign explicitly as format_int will consider
		 * -0 as 0 and doesn't print any sign, which is not true for
		 * floats.
		 * We also disable FL_PLUS flag to not allow format_int to
		 * print '+' sign.
		 * */
		if (!int_part_of_float) {
			EMIT('-');
			flags &= ~FL_PLUS;
		}
	}
	...
	/* 1. Provide width only if no. has to be right justified. i.e provide
	 *    width only if FL_MINUS is disabled
	 * 2. Width given for integer part must account for decimal pt and
	 *    remaining fractional part */
	int tmp_width = width;
	if (flags & FL_MINUS)
		tmp_width = 0;
	else if (precision)
		tmp_width = width - (precision + 1);

	int no_of_int_chars = format_int(q, n, int_part_of_float, flags, 10,
			tmp_width, -1);
	o += no_of_int_chars;
	 ...
	 * 1. Strip zero, plus, space flags
	 * 2. Provide width only if FL_MINUS is enabled */
	flags &= ~(FL_ZERO | FL_PLUS | FL_SPACE);
	tmp_width = 0;
	if (flags & FL_MINUS && width > o) {
		tmp_width = width - o;
	}

	o += format_int(q, n, frac_part_of_float, flags, 10, tmp_width,
			precision);
