...
static int tls_setup()
{
	...
	uint8_t tx_buffer[260] = {0,};
        ...
        static const char *base64_client_prvkey = keyCLIENT_PRIVATE_KEY_PEM_G3;
...
#error   "G3_CLIENT_CERT_PRIVKEY_KEYSECTOR" must be 114
        ...
        base64_address = NULL;
        base64_address = delete_header_footer((unsigned char*)keyCLIENT_PRIVATE_KEY_PEM_G3, &prvlen_base64);
	...
	ret = g3api_write_key_value(G3_CLIENT_CERT_PRIVKEY_KEYSECTOR, KEY_AREA, PLAIN_TEXT, &tx_buffer[0], size);
        ...
        cert_size = strlen(keyCLIENT_CERTIFICATE_PEM_G3) + 1;
        for(int i =0; i< (((cert_size-1)/32)+1); i++)
        {
            ...
            memcpy(tx_buffer, keyCLIENT_CERTIFICATE_PEM_G3+32*i, size);
