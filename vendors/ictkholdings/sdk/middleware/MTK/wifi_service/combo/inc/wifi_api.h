...
* The Configuration APIs include wifi_config_abc() APIs and wifi_wps_config_abc() APIs. They use in-band mechanism (in-band is a mechanism used for Wi-Fi driver and Wi-Fi firmware communication),
* thus these APIs must be called after OS task scheduler is started to make sure an in-band task is running.
* The wifi_wps_config_abc() APIs can be used to set or get default device configuration before triggering the Push Button Connection (PBC) or Personal Identification Number (PIN) process.
...
*
* The Connection APIs include wifi_connection_abc() APIs and wifi_wps_connection_abc() APIs. They manage the link status, such as disconnect AP, disconnect the station, get the link status, get the station list, start or stop the scan.
...
* The wifi_wps_connection_abc() APIs can be used to trigger the PBC or PIN process.
*
*
* Terms                         |Details                                                                 |
* ------------------------------|------------------------------------------------------------------------|
...
/**
* @brief Wi-Fi Direct feature, this feature is not supported.
*/
...
typedef enum {
    ...
    WIFI_EVENT_IOT_WPS_COMPLETE_CONNECTION_FAILED, /**< WPS Connection has failed. WPS Connection Failed event is now only supported by MT7682/7686. Please refer to #wps_status_t for the reason code. >*/
...
typedef enum {
    ...
    WIFI_REASON_CODE_BEACON_LOST = 101                    /**<  101 (Internal) disconnected as AP beacon lost. */
} wifi_reason_code_t;

/**@brief This enumeration defines the send data slow or fast indicate. Get this indicate can get the PSE buffer busy or free status.
*/
typedef enum
{
    WIFI_FLOW_GO_SLOW,                       /**< Slow Indicate, the buffer is busy, have little buffer to use. */
    WIFI_FLOW_GO_FAST                        /**< Fast Indicate, the buffer is free, have much more buffer to use. */
...
typedef enum {
    ...
    WIFI_PHY_MAX_NUMBER          /**< The max supported PHY number. */
} wifi_phy_mode_t;
...
/** @brief This enumeration defines the WPS connection status and reason code.
*/
typedef enum {
    WPS_RET_OK = 0,                         /**< WPS process was successful. */
    WPS_RET_TWO_MINS_TIMEOUT,               /**< WPS process failed because it timed-out after 2 minutes. */    
    WPS_RET_SECURITY_NOT_SUPPORT,           /**< WPS process failed. This method of security is not supproted */ 
...
/** @brief This structures provide the channel table information.
*/
typedef struct {
...
* @note only after calling #wifi_config_reload_setting().
*/
...
* Call #wifi_config_set_channel() in station mode while the device is connected to the AP, the device will disconnect from this AP and
* stay in the specified channel for a short time, then the device will scan and change the channel in a loop. \n
...
*
* @param[in] wifi_country_code is  the country code that the Wi-Fi driver uses. For more information, please refer to #wifi_country_code_t.
...
*
* @param[out] wifi_country_code contains the country code string, which is already set by #wifi_config_set_country_code(), upon the return of this function call.
*
* @return  >=0 the operation completed successfully, <0 the operation failed.
*
...
*
* @param[out]  interval is the beacon's interval in a range from 1 to 20. The beacon's interval is usually 100 times the unit (100*1.024 ms).
*
* @return  >=0 the operation completed successfully, <0 the operation failed.
*
*/
...
* This operation takes effect immediately.
*
* @param[in] interval is the beacon's interval, and it should be a multiple of AP's DTIM period. The beacon's interval is usually 100 times the unit (100*1.024 ms), which means, the listen interval range is about (1~255) * 102.4ms.
*
* @note For mt7687 and mt7697 the maximum listen interval is AP's DTIM*7, the minimum listen interval is AP's DTIM.
* For mt7682, mt7686 and mt5932 the maximum listen interval is 20, the minimum listen interval is AP's DTIM.*
*
* @return  >=0 the operation completed successfully, <0 the operation failed.
*/
...
/**
* @deprecated This function is deprecated! This function gets the PMK used in the Wi-Fi driver for a specific wireless port.
* @param[in] port is the Wi-Fi port that the function will operate on.
*
* Value                         |Definition                                                              |
* ------------------------------|------------------------------------------------------------------------|
...
* @return >=0 the operation completed successfully, <0 the operation failed.
*
* @note Migration method: please use #wifi_config_get_wpa_psk_key instead, if passphrase_length is 64, then the passphrase indicates the PMK.
*/
int32_t wifi_config_get_pmk(uint8_t port, uint8_t *pmk);

/**
* @deprecated This function is deprecated! This function sets the PMK used in the Wi-Fi driver for a specific wireless port. This operation
...
*
* @note Migration method: please use #wifi_config_set_wpa_psk_key instead, if passphrase_length is 64, then passphrase indicates the PMK,
* For example, if to set PMK[32] = {0x11,0x22,0xaa,0xbb,0x11,0x22,0xaa,0xbb,0x11,0x22,0xaa,0xbb,0x11,0x22,0xaa,0xbb,0x11,0x22,0xaa,0xbb,0x11,0x22,0xaa,0xbb,0x11,0x22,0xaa,0xbb,0x11,0x22,0xaa,0xbb} as PMK.
* Initialize passphrase_length = 64, passphrase = "1122aabb1122aabb1122aabb1122aabb1122aabb1122aabb1122aabb1122aabb"
* and call #wifi_config_set_wpa_psk_key, the setting PMK will be applied.
*
* @return >=0 the operation completed successfully, <0 the operation failed.
*/
...
/**
* @brief This function informs the Wi-Fi driver to reload the configuration and applies the
...
* and connect to the target AP router, it will continue to scan and try to connect to the target 
* router until the connection is successful. After the connection is established, if the router is 
* powered off, the system will attempt to reconnect in the background.
* In wireless AP mode, device will load the new SSID and encryption information.
*
* @return  >=0 the operation completed successfully, <0 the operation failed.
*/
...
* @return >=0 the operation completed successfully, <0 the operation failed.
*
* @note In station mode, the device will not connect to AP until the handler is unregistered.
*/
...
* @return  >=0 the operation completed successfully, <0 the operation failed.
*
* @note For MT7687/7697, this API is only supported in the station mode, AP mode or monitor mode. \n
...
/**
* @brief This function immediately disconnects the current connection from the connected AP. The device switches to
* idle state and stops scanning, regardless it's connected to the AP. After calling #wifi_config_reload_setting(), 
* the device switches to scan state and tries to connect to an AP router with an existing Wi-Fi configuration. This API should 
* be used only in the station mode.
*
* @return  >=0 the operation completed successfully, <0 the operation failed.
*
*/
...
* @return   >=0 the operation completed successfully, <0 the operation failed.
*
* @note 1. When the scan is finished, #wifi_connection_scan_deinit() should be called to unload the buffer from the Wi-Fi driver. After that, the data in the parameter ap_list can be safely processed by user applications, and then another scan can be initialized by calling #wifi_connection_scan_init().
* @note 2. Remember to call #wifi_connection_scan_deinit() to free resources before the task (which triggers the scan init)is destroyed..
*/
...
*
*
* @return  ==0 the operation completed successfully, <0 or >0 the operation failed.
*
...
* @note 3. The AP/repeater mode supports only the partial scan mode.
*/
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores the operation mode in a profile in the Flash memory. \n
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function gets the operation mode from a profile in the Flash memory.
*
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores the bandwidth in a profile in the Flash memory. \n
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function gets the MAC address of the Wi-Fi interface from a profile in the Flash memory.
*
* @param[in] port indicates the Wi-Fi port that the function will operate on.
*
* Value                         |Definition                                                              |
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function sets the MAC address of the Wi-Fi interface in a profile in the Flash memory.
* It uses the same profile section for a specific wireless port, such as #WIFI_PORT_STA / #WIFI_PORT_APCLI.
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores the SSID in a profile in the Flash memory. \n
* It uses the same profile section for a specific wireless port, such as #WIFI_PORT_STA / #WIFI_PORT_APCLI.
* The profile is read to initialize the system at boot up.
*
* @param[in] port indicates the Wi-Fi port that the function will operate on.
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores the Wi-Fi Wireless Mode in a profile in the Flash memory. \n
* It uses the same profile section for a specific wireless port, such as #WIFI_PORT_STA / #WIFI_PORT_APCLI.
* The profile is read to initialize the system at boot up.
*
* @param[in] port indicates the Wi-Fi port that the function will operate on.
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores the Wi-Fi authentication mode and encryption type in a profile in the Flash memory.
* It uses the same profile section for a specific wireless port, such as #WIFI_PORT_STA / #WIFI_PORT_APCLI.
* The profile is read to initialize the system at boot up.
*
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores the Wi-Fi WPAPSK/WPA2PSK passphrase in a profile in the Flash memory. \n
* It uses the same profile section for a specific wireless port, such as #WIFI_PORT_STA / #WIFI_PORT_APCLI.
* The profile is read to initialize the system at boot up.
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function gets the PMK from a profile in the Flash memory.
* @param[in] port indicates the Wi-Fi port that the function will operate on.
*
* Value                         |Definition                                                              |
* ------------------------------|------------------------------------------------------------------------|
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores Wi-Fi WEP Keys in a profile in the Flash memory.
* It uses the same profile section for a specific wireless port, such as #WIFI_PORT_STA / #WIFI_PORT_APCLI.
* The profile is read to initialize the system at boot up.
*
* @param[in] port indicates the Wi-Fi port that the function will operate on.
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function gets the Wi-Fi Country Region used in the Wi-Fi driver.
* Various world regions have allocated different amounts of frequency space
* around 2.4GHz for Wi-Fi and some of them have fewer or more channels
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function sets the Wi-Fi Country Region in a profile in the Flash memory.
* Various world regions have allocated different amounts of frequency space
* around 2.4GHz for Wi-Fi and some of them have fewer or more channels
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function gets the Wi-Fi DTIM interval from a profile in the Flash memory.
*
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function sets the Wi-Fi DTIM interval in a profile in the Flash memory. This API can be used only for STA mode.
* The profile is read to initialize the system at boot up.
*
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function gets the Wi-Fi Listen Interval from a profile in the Flash memory.
*
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function sets the Wi-Fi Listen Interval in a profile in the Flash memory. This API can be used only for STA mode.
* The profile is read to initialize the system at boot up.
*
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function stores the Wi-Fi Power Save mode in a profile in the Flash memory. \n
* The profile is read to initialize the system at boot up.
*
...
/**
* @deprecated This function is deprecated! For more information, please refer to Airoha IoT SDK for Wi-Fi Migration Developer's Guide under the doc folder in the SDK package. This function is not implemented.
*/
