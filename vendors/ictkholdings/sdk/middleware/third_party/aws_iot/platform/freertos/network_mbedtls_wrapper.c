...
//#include "aws_iot_cert_rtos.h"                //dkyeo0508
...
#ifdef ICTK_ZTP
unsigned char * signerca_ecc_cert = NULL;
uint16_t len_signerca_ecc_cert = 0;
...
static int _iot_tls_verify_cert(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags) {
    ...
    buf_ptr = (char*) pvPortCalloc(1, 1024);
    if (buf_ptr == NULL) {        
...
//------------------------------------------------------------------------------
    ...
    client_ecc_cert = (unsigned char*) mbedtls_calloc((sect_end - sect_start + 1), sizeof(ST_KEY_VALUE));
    ...
    if (ret != 0)
    {
      ...
      return FAILURE;
		...
		//---SINGER CA CRT ---
		ret = get_tls_profile(SIGNER_CA, 7, out_tls_client_key_information,sizeof(out_tls_client_key_information));
		size	   = (out_tls_client_key_information[28] << 8) + out_tls_client_key_information[29];
		sect_start = out_tls_client_key_information[30];
		sect_end   = out_tls_client_key_information[31];
		mbedtls_printf("\n	. Get the size of certificate for client...size[%d],sect start[%d],sect end[%d]", size,sect_start,sect_end);
		...
		ret = mbedtls_x509_crt_parse(&(tlsDataParams->clicert), (const unsigned char *)signerca_ecc_cert, len_signerca_ecc_cert);
    ...
    ca_ecc_cert = (unsigned char*) mbedtls_calloc((sect_end - sect_start + 1), sizeof(ST_KEY_VALUE));
