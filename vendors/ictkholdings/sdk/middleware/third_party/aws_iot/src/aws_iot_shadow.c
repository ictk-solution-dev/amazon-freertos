...
#if 1 // Dummy
	mqttInitParams.pRootCALocation = pParams->pHost;
	mqttInitParams.pDeviceCertLocation = pParams->pHost;
	mqttInitParams.pDevicePrivateKeyLocation = pParams->pHost;
#else
...
#ifdef ICTK_ZTP
static int send_signer_ca_flag_ = 0;
void ztp_set_send_signer_ca_flag(int send_signer_ca_flag){
	send_signer_ca_flag_ = send_signer_ca_flag;
}
int ztp_get_send_signer_ca_flag(){
	return send_signer_ca_flag_;
}
#endif
#ifdef __cplusplus
}
