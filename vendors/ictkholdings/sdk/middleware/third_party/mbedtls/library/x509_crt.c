                        ...
                        size_t buflen )
{
    ...
    {
        crt->next = (struct mbedtls_x509_crt*)mbedtls_calloc( 1, sizeof( mbedtls_x509_crt ) );
                     ...
                     mbedtls_x509_crt_restart_ctx *rs_ctx )
{
    ...
    *flags = 0;
...
#ifdef ICTK_TLS_CHECK_CN_NAME	//added by ICTK
    ...
    if( x509_profile_check_pk_alg( profile, pk_type ) != 0 ){
		...
		printf("x509_profile_check_pk_alg not ok");
        ee_flags |= MBEDTLS_X509_BADCERT_BAD_PK;
    }

    if( x509_profile_check_key( profile, &crt->pk ) != 0 ){
		
		printf("x509_profile_check_key not ok");
