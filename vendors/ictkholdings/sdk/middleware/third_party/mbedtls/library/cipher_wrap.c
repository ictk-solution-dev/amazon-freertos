...
{
    void *ctx = (void*)mbedtls_calloc( 1, sizeof( mbedtls_gcm_context ) );
...
{
    void *ctx = (void *)mbedtls_calloc( 1, sizeof( mbedtls_ccm_context ) );
        ...
        const unsigned char *input, unsigned char *output )
{
  int ret = 0;
	...
	ret = mediatek_aes_crypt_ecb( (mbedtls_aes_context *) ctx, operation, input, output );

  return ret;
#else
#if defined(ICTK_TLS) && !defined(ICTK_TLS_PREMASTER_IN_PUF)
	int usage;

	if(operation == MBEDTLS_ENCRYPT)
		usage = ICTKTLS_AES_USAGE_ENC_DATA;
	else if(operation == MBEDTLS_DECRYPT)		
		usage = ICTKTLS_AES_USAGE_DEC_DATA;
	else
		return -1;
	...
	if(get_support_ciphersuite() == 0)
		...
		return ictktls_aes_crypt_ecb( operation, usage,input, output );
#else

	ret = mbedtls_aes_crypt_ecb( (mbedtls_aes_context *) ctx, operation, input, output );
  return ret;
#endif
#endif
}
        ...
        unsigned char *iv, const unsigned char *input, unsigned char *output )
{
...
#endif
#ifdef MEDIATEK_AES_ENGINE
	return mediatek_aes_crypt_cbc( (mbedtls_aes_context *) ctx, operation, length, iv, input,
						  output );
#else
#if defined(ICTK_TLS) && !defined(ICTK_TLS_PREMASTER_IN_PUF)
	int usage;
	AES_ECB_USE ecb_use = ECB_NONE_USE;
	...
	if(get_support_ciphersuite() == 0){
    	...
    	return ictktls_aes_crypt_cbc( operation, usage,ecb_use,length, iv, input,
                          output );
	}
#else
	return mbedtls_aes_crypt_cbc( (mbedtls_aes_context *) ctx, operation, length, iv, input,
...
#endif
#ifdef MEDIATEK_AES_ENGINE
	return mediatek_aes_setkey_dec((mbedtls_aes_context *) ctx, key, key_bitlen);
#else
#if defined(ICTK_TLS) && !defined(ICTK_TLS_PREMASTER_IN_PUF)
	if(get_support_ciphersuite() == 0)
    	return mbedtls_aes_setkey_dec( (mbedtls_aes_context *) ctx, key, key_bitlen );
	else
    	return ictktls_aes_setkey_enc(ICTKTLS_AES_USAGE_DEC_DATA, key, key_bitlen);
#else
    return mbedtls_aes_setkey_dec( (mbedtls_aes_context *) ctx, key, key_bitlen );
#endif
#endif
...
#endif
#ifdef MEDIATEK_AES_ENGINE
	return mediatek_aes_setkey_enc((mbedtls_aes_context *) ctx, key, key_bitlen);
#else
#if defined(ICTK_TLS)&& !defined(ICTK_TLS_PREMASTER_IN_PUF)
	if(get_support_ciphersuite() == 0)
          return mbedtls_aes_setkey_enc( (mbedtls_aes_context *) ctx, key, key_bitlen);
        else
          return ictktls_aes_setkey_enc(ICTKTLS_AES_USAGE_ENC_DATA, key, key_bitlen);
#else
	return mbedtls_aes_setkey_enc( (mbedtls_aes_context *) ctx, key, key_bitlen);
#endif
#endif
