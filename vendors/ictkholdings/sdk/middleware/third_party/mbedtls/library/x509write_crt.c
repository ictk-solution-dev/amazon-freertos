...
int mbedtls_x509write_crt_set_subject_key_identifier( mbedtls_x509write_cert *ctx )
{
    ...
    memset( buf, 0, sizeof(buf) );
#ifdef ICTK_ZTP
    MBEDTLS_ASN1_CHK_ADD( len, ictktls_pk_write_pubkey( &c, buf, ctx->subject_key ) );
#else
...
int mbedtls_x509write_crt_set_subject_key_identifier_2( mbedtls_x509write_cert *ctx , uint8_t* subjectKey)
{
	int ret;
	unsigned char buf[MBEDTLS_MPI_MAX_SIZE * 2 + 20]; /* tag, length + 2xMPI */
	unsigned char *c = buf + sizeof(buf);
                       ...
                       void *p_rng )
{
     ...
     */
#ifdef ICTK_ZTP
    MBEDTLS_ASN1_CHK_ADD( pub_len, ictktls_pk_write_pubkey_der( ctx->subject_key,
                                                tmp_buf, c - tmp_buf ) );
