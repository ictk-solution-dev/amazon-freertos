...
static 
#endif
void ssl_calc_verify_tls_sha256( mbedtls_ssl_context *,unsigned char * );
#if !defined(ICTK_TLS)
static 
#endif
void ssl_calc_finished_tls_sha256( mbedtls_ssl_context *,unsigned char *, int );
#endif
...
int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
...
#ifdef ICTK_TLS
	#ifndef ICTK_TLS_PREMASTER_IN_PUF
     ...
     */
#if defined(ICTK_TLS_PREMASTER_IN_PUF)//!defined(ICTK_TLS)
...
static int ssl_encrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        int ret;
        unsigned char *enc_msg;
	#if !defined(ICTK_TLS_PREMASTER_IN_PUF)
		size_t enc_msglen, padlen, olen = 0, i;
	#else
		size_t enc_msglen, olen = 0;
	#endif
	#if !defined(ICTK_TLS_PREMASTER_IN_PUF)
        ...
        if( ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_2 )
        {
            ...
            if( ret != 0 )
                return( ret );
			MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK] random number iv enc", ssl->transform_out->iv_enc, ssl->transform_out->ivlen );
            ...
            enc_msg = ssl->out_msg;
            enc_msglen = ssl->out_msglen;
			MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]enc msg ==> random number", enc_msg, enc_msglen ); //add the description by hupark...enc_msg output payload 16 byte
#if !defined(ICTK_TLS_PREMASTER_IN_PUF)
		...
		memcpy( pseudo_hdr +  8, ssl->out_hdr, 3 ); //add the comment by hupark..msgtype+version...
		pseudo_hdr[11] = (unsigned char)( ( enc_msglen >> 8 ) & 0xFF );
		pseudo_hdr[12] = (unsigned char)( ( enc_msglen 	 ) & 0xFF );

		MBEDTLS_SSL_DEBUG_BUF( 4, "[ICTK]MAC'd meta-data", pseudo_hdr, 13 );
		...
		mbedtls_md_hmac_finish( &ssl->transform_out->md_ctx_enc, mac );
        mbedtls_md_hmac_reset( &ssl->transform_out->md_ctx_enc );
		MBEDTLS_SSL_DEBUG_BUF( 4, "[ICTK]mac ", mac, MBEDTLS_SSL_MAC_ADD );

		memcpy(&plain[0], random, 16 );
		plain_len += 16;
		memcpy(&plain[16], enc_msg, enc_msglen );
		plain_len += enc_msglen;

		memcpy(&plain[16 + enc_msglen], mac, MBEDTLS_SSL_MAC_ADD );
		plain_len += MBEDTLS_SSL_MAC_ADD;
        ...
        for( i = 0; i <= padlen; i++ )
            plain[plain_len + i] = (unsigned char) padlen;

		plain_len = plain_len + padlen + 1;
		MBEDTLS_SSL_DEBUG_BUF( 4, "[ICTK]plain msg ", plain, plain_len );
		
        if( ( ret = mbedtls_cipher_crypt( &ssl->transform_out->cipher_ctx_enc,
                                   ssl->transform_out->iv_enc,
                                   ssl->transform_out->ivlen,
                                   plain, plain_len,
                                   enc_msg, &crypto_len ) ) != 0 )
        {
            MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_cipher_crypt", ret );
            return( ret );
        }
		...
		memcpy(ssl->out_iv + ssl->transform_out->ivlen , enc_msg,  crypto_len);
		MBEDTLS_SSL_DEBUG_BUF( 4, "[ICTK]enc msg ", ssl->out_iv, ssl->out_msglen );
        auth_done++;
		...
		MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]transform out ==> iv enc", ssl->transform_out->iv_enc, ssl->transform_out->ivlen ); 

		MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]enc msg ", enc_msg, olen );
        ...
        if( auth_done == 0 )
        {
            ...
            ssl->out_msglen += ssl->transform_out->maclen;
            auth_done++;
			MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]total encrypted data", ssl->out_iv, ssl->out_msglen);
			MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]mac data", mac,  ssl->transform_out->maclen);
			//iv + enc data + mac data
        }
...
static int ssl_decrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        ...
        if( ssl->session_in->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED )
        {
			...
			MBEDTLS_SSL_DEBUG_BUF( 4, "IN IV", ssl->in_iv, ssl->in_msglen );
...
#endif	/*ICTK_TLS_PREMASTER_IN_PUF*/
#endif /* MBEDTLS_SSL_ENCRYPT_THEN_MAC */
		...
		size_t plain_len;
		  ...
		  if( ( ret = mbedtls_cipher_crypt( &ssl->transform_in->cipher_ctx_dec,
		   ...
		   plain_len = olen - (16 + 32 + padlen);
		   //memcpy(&ssl->in_msg[0] , dec_msg_result + 16 , plain_len);
		   ssl->in_msglen = plain_len;
		   ...
		   if( ssl->session_in->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED )
		   {
			   ...
			   memcpy( pseudo_hdr +  0, ssl->in_ctr, 8 );
			   memcpy( pseudo_hdr +  8, ssl->in_hdr, 3 );
			   pseudo_hdr[11] = (unsigned char)( ( plain_len >> 8 ) & 0xFF );
			   pseudo_hdr[12] = (unsigned char)( ( plain_len	  ) & 0xFF );
	
			   MBEDTLS_SSL_DEBUG_BUF( 4, "MAC'd meta-data", pseudo_hdr, 13 );
	
			   mbedtls_md_hmac_update( &ssl->transform_in->md_ctx_dec, pseudo_hdr, 13 );
			   mbedtls_md_hmac_update( &ssl->transform_in->md_ctx_dec,
								dec_msg_result + 16, plain_len );
			   MBEDTLS_SSL_DEBUG_BUF( 4, "IN IV", dec_msg_result + 16, plain_len );
			   mbedtls_md_hmac_finish( &ssl->transform_in->md_ctx_dec, mac_expect );
			   mbedtls_md_hmac_reset( &ssl->transform_in->md_ctx_dec );
	
			   MBEDTLS_SSL_DEBUG_BUF( 4, "message  mac", dec_msg_result + (16 + plain_len),
												 MBEDTLS_SSL_MAC_ADD );
			   MBEDTLS_SSL_DEBUG_BUF( 4, "expected mac", mac_expect,
												 MBEDTLS_SSL_MAC_ADD );
	
			   if( mbedtls_ssl_safer_memcmp( dec_msg_result + (16 + plain_len), mac_expect,
											 MBEDTLS_SSL_MAC_ADD ) != 0 )
			   {
				   MBEDTLS_SSL_DEBUG_MSG( 1, ( "message mac does not match" ) );
	
				   return( MBEDTLS_ERR_SSL_INVALID_MAC );
			   }else{		   
				   MBEDTLS_SSL_DEBUG_MSG( 1, ( "message mac match" ) );
				   correct = 1;
			   }
			   auth_done++;
		   }
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        else
        {
            ...
            if( ssl->f_recv_timeout != NULL )
                ...
                ret = ssl->f_recv( ssl->p_bio, ssl->in_hdr, len ); //receive data....
...
int mbedtls_ssl_flush_output( mbedtls_ssl_context *ssl )
{
    ...
    while( ssl->out_left > 0 )
    {
        ...
        ret = ssl->f_send( ssl->p_bio, buf, ssl->out_left ); //send data....
...
static void ssl_swap_epochs( mbedtls_ssl_context *ssl )
{
	...
	MBEDTLS_SSL_DEBUG_MSG( 3, ( "[HUPARK]swap epochs" ) );
...
int mbedtls_ssl_write_handshake_msg( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE )
    {
        ...
        if( hs_type != MBEDTLS_SSL_HS_HELLO_REQUEST ){
			MBEDTLS_SSL_DEBUG_BUF( 3, "[HUPARK]mbedtls_ssl_write_handshake_msg => update checksum",
                              ssl->out_msg,
                              ssl->out_msglen );
            ssl->handshake->update_checksum( ssl, ssl->out_msg, ssl->out_msglen ); //hupark...
        }
    }
    ...
    if( !done )
    {
        ...
        if( ssl->transform_out != NULL )
        {
			...
			if(get_support_ciphersuite() == 0){
				...
				if( ( ret = ictk_ssl_encrypt_buf( ssl ) ) != 0 )
				{
					MBEDTLS_SSL_DEBUG_RET( 1, "ssl_encrypt_buf", ret );
					return( ret );
				}
		...
		MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]current output ctr",
									   ssl->cur_out_ctr, 8);

        for( i = 8; i > ssl_ep_len( ssl ); i-- ) //Increase sequence number...hupark...
            if( ++ssl->cur_out_ctr[i - 1] != 0 ){
				MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]next output ctr ",
                               ssl->cur_out_ctr, 8);
                break;
            }
...
static int ssl_prepare_record_content( mbedtls_ssl_context *ssl )
{
    ...
    if( !done && ssl->transform_in != NULL )
    {
		...
		if( ( ret = ictk_ssl_decrypt_buf( ssl ) ) != 0 )
		{
			MBEDTLS_SSL_DEBUG_RET( 1, "ssl_decrypt_buf", ret );
			return( ret );
		}
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
	...
	if( ssl->handshake != NULL && ssl->handshake->key_cert != NULL )
        MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= write certificate ==>handshake key cert exists" ) );
    else
        MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= write certificate ==>con key cert exists" ) );
	...
	if(crt == NULL){
		MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= write certificate ==>own cert is null" ) );
	}
...
crt_verify:
    ...
    if( authmode != MBEDTLS_SSL_VERIFY_NONE )
    {
         ...
         */
		MBEDTLS_SSL_DEBUG_CRT( 3, "ca_chain certificate", ca_chain );
...
#endif /* MBEDTLS_SSL_PROTO_DTLS */
    memset( ssl->in_ctr, 0, 8 ); //important...by hupark
...
static void ssl_update_checksum_sha256( mbedtls_ssl_context *ssl,
                                        const unsigned char *buf, size_t len )
{
	MBEDTLS_SSL_DEBUG_MSG( 2, ( "=> [HUPARK]ssl_update_checksum_sha256" ) );
    MBEDTLS_SSL_DEBUG_BUF( 4, "ssl_update_checksum_sha256", (unsigned char *)
                    buf, len );


    mbedtls_sha256_update_ret( &ssl->handshake->fin_sha256, buf, len );
	MBEDTLS_SSL_DEBUG_BUF( 4, "ssl_update_checksum_sha256", (unsigned char *)
                    &ssl->handshake->fin_sha256.buffer, 64 );
}
#endif
...
static 
#endif
void ssl_calc_finished_tls_sha256(
                mbedtls_ssl_context *ssl, unsigned char *buf, int from )
{
    int len = 12;
    const char *sender;
    ...
    MBEDTLS_SSL_DEBUG_BUF( 4, "[HUPARK]ssl_calc_finished_tls_sha256 ", (unsigned char const*)&ssl->handshake->fin_sha256.buffer, 64 );
...
#endif /* MBEDTLS_SSL_PROTO_DTLS */
    memset( ssl->cur_out_ctr, 0, 8 ); //initialize ctr..hupark
...
int mbedtls_ssl_write( mbedtls_ssl_context *ssl, const unsigned char *buf, size_t len )
{
    ...
    if( ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER )
    {
    	...
    	MBEDTLS_SSL_DEBUG_MSG( 2, ( "=> state is not MBEDTLS_SSL_HANDSHAKE_OVER" ) );
    }
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
#ifdef ICTK_TLS
	conf->extended_ms = MBEDTLS_SSL_EXTENDED_MS_DISABLED;
#else
...
#ifdef ICTK_TLS
			retVal = set_support_ciphersuite(MBEDTLS_SSL_CIPHERSUITES/*0xC0AC/*0xC02B*//*0xC023/*MBEDTLS_SSL_CIPHERSUITES*/);
#endif
                                            ...
                                            mbedtls_md_type_t md_alg )
{
    ...
    *hashlen = mbedtls_md_get_size( md_info );
    ...
    mbedtls_md_init( &ctx );
	MBEDTLS_SSL_DEBUG_MSG( 1, ( "mbedtls_ssl_get_key_exchange_md_tls1_2 is called" ) );
     ...
     * };
     */
    MBEDTLS_SSL_DEBUG_MSG( 1, ( "mbedtls_md_setupis called" ) );
    ...
    MBEDTLS_SSL_DEBUG_MSG( 1, ( "mbedtls_md_starts called" ) );
    if( ( ret = mbedtls_md_starts( &ctx ) ) != 0 )
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_md_starts", ret );
        goto exit;
    }
    MBEDTLS_SSL_DEBUG_MSG( 1, ( "mbedtls_md_update1 called" ) );
    ...
    MBEDTLS_SSL_DEBUG_MSG( 1, ( "mbedtls_md_update2 called" ) );
    if( ( ret = mbedtls_md_update( &ctx, data, data_len ) ) != 0 )
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_md_update", ret );
        goto exit;
    }
	MBEDTLS_SSL_DEBUG_MSG( 1, ( "mbedtls_md_finish called" ) );
...
exit:
    ...
    return( ret );
          ...
          MBEDTLS_SSL_PROTO_TLS1_2 */
#ifdef ICTK_TLS
static int support_ciphersurite = 0;
int set_support_ciphersuite(const int ciphersuite ){

	if((const int)0xC023/*MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256*/== ciphersuite){	
	#ifdef MEDIATEK_AES_ENGINE
		support_ciphersurite = 0 ;
	#else
		support_ciphersurite = 1 ;
	#endif
	}else if((const int)0xC025 /*MBEDTLS_TLS_ECDH_ECDSA_WITH_AES_128_CBC_SHA256*/== ciphersuite){	
		support_ciphersurite = 0 ;
	}else if((const int)0xC02B/*MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256*/ == ciphersuite){	
		support_ciphersurite = 0 ;
	}else if((const int)0xC0AC /*MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_128_CCM*/== ciphersuite){	
		support_ciphersurite = 0 ;
	}else if((const int)0xC048/*MBEDTLS_TLS_ECDHE_ECDSA_WITH_ARIA_128_CBC_SHA256*/== ciphersuite){	
		support_ciphersurite = 0 ;
	}else if((const int)0xC0AD /*MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_256_CCM*/== ciphersuite){
		support_ciphersurite = 0 ;
	}else if((const int)0xC0AF /*MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_256_CCM_8*/== ciphersuite){
		support_ciphersurite = 0 ;  
	}else if((const int)0xC05C /*MBEDTLS_TLS_ECDHE_ECDSA_WITH_ARIA_128_GCM_SHA256*/== ciphersuite){
		support_ciphersurite = 0 ;  
	}else if((const int)0xC048 /*MBEDTLS_TLS_ECDHE_ECDSA_WITH_ARIA_128_CBC_SHA256*/== ciphersuite){
		support_ciphersurite = 0 ;  
	}else if((const int)0xC072 /*MBEDTLS_TLS_ECDHE_ECDSA_WITH_CAMELLIA_128_CBC_SHA256*/== ciphersuite){
		support_ciphersurite = 0 ;  
	}else if((const int)0xCCA9 /*MBEDTLS_TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256*/== ciphersuite){
		support_ciphersurite = 0 ;  
	}else
		support_ciphersurite = 0;
	//mbedtls_printf("\n  .set_support_ciphersuite %d", support_ciphersurite);
	return support_ciphersurite;
}
int get_support_ciphersuite(){
	return support_ciphersurite;
}
#endif
