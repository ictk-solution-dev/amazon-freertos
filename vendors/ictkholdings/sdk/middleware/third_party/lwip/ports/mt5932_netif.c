...
#endif
static int32_t mt5932_low_level_output(struct netif *netif, struct pbuf *p);
static void mt5932_low_level_init1(struct netif *netif);
static void mt5932_low_level_init2(struct netif *netif);
static err_t mt5932_netif_init1(struct netif *netif);
static err_t mt5932_netif_init2(struct netif *netif);
static void
mt5932_ethernetif_init(ip4_addr_t *sta_ip_addr, ip4_addr_t *sta_net_mask, ip4_addr_t *sta_gw,
...
void
mt5932_low_level_input(struct netif *netif, struct pbuf *p)
{
...
static int32_t
mt5932_low_level_output(struct netif *netif, struct pbuf *p)
{
    struct pbuf *q;
 ...
 */
static void
mt5932_low_level_init1(struct netif *netif)
{
...
static void
mt5932_low_level_init2(struct netif *netif)
{
 ...
 */
static err_t
mt5932_netif_init1(struct netif *netif)
{
  ...
  #endif /* LWIP_IPV6 */
  netif->linkoutput = (netif_linkoutput_fn)mt5932_low_level_output;

  ethernetif->ethaddr = (struct eth_addr *)&(netif->hwaddr[0]);

  /* initialize the hardware */
  mt5932_low_level_init1(netif);
...
err_t
mt5932_netif_init2(struct netif *netif)
{
  struct ethernetif *ethernetif;
...
mt5932_ethernetif_init(ip4_addr_t *sta_ip_addr, ip4_addr_t *sta_net_mask, ip4_addr_t *sta_gw,
                             ...
                             uint8_t opmode)
{
    uint8_t sta_mac_address[6] = {0};
    uint8_t ap_mac_address[6]  = {0};

    wifi_host_config_get_mac_address(WIFI_PORT_STA, sta_mac_address);
    wifi_host_config_get_mac_address(WIFI_PORT_AP, ap_mac_address);

    printf("[["MACSTR"] \n", MAC2STR(sta_mac_address));
    printf("["MACSTR"] \n", MAC2STR(ap_mac_address));
    ...
    */
    ...
    netif_add(&sta_if, sta_ip_addr, sta_net_mask, sta_gw,
              NULL, mt5932_netif_init1, tcpip_input);
    netif_add(&ap_if, ap_ip_addr, ap_net_mask, ap_gw,
              NULL, mt5932_netif_init2, tcpip_input);
    ...
    //wlan_rx_register_callback(mt5932_low_level_input, &sta_if, &ap_if);
...
{
    mt5932_ethernetif_init(&tcpip_config->sta_addr,
