...
#elif defined(MTK_ONLINE_LOG_TO_APK_ENABLE)
#define port_syslog_device_early_init(port_type, port_index, baudrate)          g_syslog_device_ops_array[LOG_PORT_TYPE_ONLINE_BT]->early_init(port_index, baudrate)
#define port_syslog_device_early_send(port_type, port_index, buffer, size)      g_syslog_device_ops_array[LOG_PORT_TYPE_ONLINE_BT]->early_send(port_index, buffer, size)
#else
