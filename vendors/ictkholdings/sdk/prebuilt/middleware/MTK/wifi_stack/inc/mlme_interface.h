...
#define STA_NO_CONNECT_POWER_ON     1
#define STA_NO_CONNECT_POWER_OFF    0
...
extern int fw_mlme_power_save_mode(uint8_t ps_mode);

extern int fw_mlme_get_inf_by_addr(uint8_t * addr);

extern int fw_mlme_set_sys_sleep(uint8_t opmode ,uint8_t on_off);

extern int fw_mlme_get_sys_sleep(uint8_t *on_off);

extern int fw_mlme_send_raw_pkt(uint8_t on_off);
...
extern int fw_mlme_trigger_wps_done(uint8_t port, uint8_t status);
...
extern int fw_mlme_wps_terminate_connect(int32_t port);

extern void __wps_trigger_handler_sta(void *data);
