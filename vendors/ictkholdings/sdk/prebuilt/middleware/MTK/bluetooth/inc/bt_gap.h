 ...
 * @param[in] max_count                  is the maximum device count of the inquiry, stops the inquiry, if the number of searched devices reaches the max_count.
                                         The range is from 0x00 to 0xFF, 0 - unlimited number of responses.
    @param[in] lap                          is the LAP vaule of the inquiry, Please refer to <a href="https://www.bluetooth.com/specifications/assigned-numbers/baseband">The DIACs field</a>.
 ...
 * @param[in] handle                     is the link handle to be disconnected.
 * @param[in] reason                     is the reason to disconnect the link. Only these reasons including 
 ...
 * @return    A pointer to the pin information. A NULL pointer means that user wants to negatively reply the pin code.
 */
const bt_gap_pin_code_information_t* bt_gap_get_pin_code(void);
...
/**
 * @brief     This function writes the inquiry access code. The #BT_GAP_WRITE_INQUIRY_ACCESS_CODE_CNF event is received once the operation is complete.
 * @param[in] iac_lap           is the array of IAC LAP value, Please refer to <a href="https://www.bluetooth.com/specifications/assigned-numbers/baseband">The DIACs field</a>.
                                The range is from 0x0x9E8B00 to 0x9E8B3F.
 * @param[in] num               is the array number of iac array.
...
/**
 * @brief     This function reads the remote version. The #BT_GAP_READ_REMOTE_VERSION_COMPLETE_IND event is received once the operation is complete.
 * @param[in] handle     is the link handle.
 * @return               #BT_STATUS_SUCCESS, the operation completed successfully.
 *                       #BT_STATUS_FAIL, an error occurred, the operation has terminated.
 *                       #BT_STATUS_CONNECTION_NOT_FOUND, the operation has failed, the link does not exist.
